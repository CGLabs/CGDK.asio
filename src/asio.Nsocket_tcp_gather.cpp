...
asio::Nsocket_tcp_gather::~Nsocket_tcp_gather() noexcept
{
	// 1) complete close socket
	this->process_closesocket(boost::asio::error::connection_aborted);
}

void asio::Nsocket_tcp_gather::process_closesocket(boost::system::error_code _error_code) noexcept
{
	// 1) abortive close
...
bool asio::Nsocket_tcp_gather::process_send(SEND_NODE&& _send_node)
{
	// check) 
...
void asio::Nsocket_tcp_gather::process_send_async(const SEND_NODE& _send_node)
{
	// 1) hold self
	if (!this->m_hold_send)
