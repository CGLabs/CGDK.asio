...
void group::list<_TMEMBER, _TPARAM>::_process_attach_member(std::shared_ptr<member_t> _pmember)
{
	...
	Igroup<_TMEMBER>::member_put_iter<typename group::list<_TMEMBER, _TPARAM>::iterator_t>(_pmember.get(), --this->m_container_member.end());
...
{
	// 1) erase from list
	this->m_container_member.erase(Igroup<_TMEMBER>::member_get_iter<typename group::list<_TMEMBER, _TPARAM>::iterator_t>(_pmember));
